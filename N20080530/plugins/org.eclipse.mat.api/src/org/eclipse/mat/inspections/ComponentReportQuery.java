...
import org.eclipse.mat.inspections.query.collections.CollectionFillRatioQuery;
...
public class ComponentReportQuery implements IQuery
{
    ...
    private void addDuplicateStrings(SectionSpec componentReport, Histogram histogram, Ticks listener) throws Exception
    {
        for (ClassHistogramRecord record : histogram.getClassHistogramRecords())
        {
            ...
            IResultTable groupByResult = (IResultTable) QueryUtil.execute(groupBy, listener);

            RefinedStructuredResult.Builder builder = new RefinedStructuredResult.Builder(snapshot, groupByResult);
                    ...
                    throws Exception
    {
        ...
        for (ClassHistogramRecord record : histogram.getClassHistogramRecords())
        {
            if (metadata.containsKey(record.getClassId()))
            {
                ...
                IResultTable table = (IResultTable) QueryUtil.execute(bySize, listener);

                // refine result: sort & evaluate
                RefinedStructuredResult.Builder builder = new RefinedStructuredResult.Builder(snapshot, table);
                builder.setInlineRetainedSizeCalculation(true);
                    ...
                    Ticks listener) throws Exception
    {
        ...
        for (CollectionUtil.Info info : CollectionUtil.knownCollections)
        {
            if (info.getSizeField() == null || info.getArrayField() == null)
                continue;
        ...
        for (ClassHistogramRecord record : histogram.getClassHistogramRecords())
        {
            if (metadata.containsKey(record.getClassId()))
            {
                ...
                // run the query: collections by size
                CollectionFillRatioQuery query = new CollectionFillRatioQuery();
                query.snapshot = snapshot;
                query.objects = QueryUtil.asArgument(record.getObjectIds());
                IResultTable table = (IResultTable) QueryUtil.execute(query, listener);
                    ...
                    Ticks listener) throws Exception
    {
        ...
        for (ClassHistogramRecord record : histogram.getClassHistogramRecords())
        {
            if (metadata.containsKey(record.getClassId()))
            {
                ...
                // run the query: collections by size
                MapCollisionRatioQuery query = new MapCollisionRatioQuery();
                query.snapshot = snapshot;
                ...
                query.objects = QueryUtil.asArgument(objectIds);
                IResultTable table = (IResultTable) QueryUtil.execute(query, listener);
