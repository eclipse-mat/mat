...
public class IBMExecDumpProvider extends BaseProvider
{
    ...
    private static final String EXEC_DESCRIPTION_PREFIX = "Exec "; //$NON-NLS-1$
    ...
    {
        listener.beginTask(Messages.getString("IBMExecDumpProvider.GeneratingDump"), 1000); //$NON-NLS-1$
        ProcessBuilder pb = new ProcessBuilder();
        Process p = null;
        String home = getJavaHome();
        File javaExec;
        javaExec = javaExec(home);
        String vm = info.getPid() + "," + info.getDescription().substring(EXEC_DESCRIPTION_PREFIX.length()); //$NON-NLS-1$
        String jar = getExecJar().getAbsolutePath();
        pb.command(javaExec.getPath(), "-jar", jar, vm, info.getProposedFileName()); //$NON-NLS-1$
        p = pb.start();
    ...
    public List<VmInfo> getAvailableVMs()
    {
        ...
        {}
        /*
         * 1.Try previous/no directory
         * 2.Query directory - based on previous/Java.home 
         * 3.If no directory/failed, offer abort/retry/ignore
         */
    ...
    private String defaultJavaHome()
    {
        ...
        if (path != null) {
            for (String p : path.split(File.pathSeparator))
            {
                ...
                if (dll.exists()) {
                    if (parentDir != null) return parentDir.getPath();
                }
                dll = new File(p, "dgcollector.so"); //$NON-NLS-1$
                if (dll.exists()) {
                    if (parentDir != null) return parentDir.getPath();
                }
                // Perhaps we were given the sdk/bin directory, so look for the sdk/jre/bin
    ...
    private List<VmInfo> execGetVMs(File javaExec, String home)
    {
        ...
        try {
            String jar = getExecJar().getAbsolutePath();
            pb.command(javaExec.getPath(), "-jar", jar); //$NON-NLS-1$
            p = pb.start();
            StringBuffer err = new StringBuffer();
            StringBuffer in = new StringBuffer();
            InputStreamReader os = new InputStreamReader(p.getInputStream());
    ...
    static synchronized File getExecJar() throws IOException {
        if (execJar == null || !execJar.canRead())
        {
            ...
            String classesNames[] = {"org.eclipse.mat.ibmvm.acquire.VMListDumpProvider", //$NON-NLS-1$
                               "org.eclipse.mat.ibmvm.acquire.IBMDumpProvider", //$NON-NLS-1$
            ...
            Class<?> classes[] = {
                            ...
                            IHeapDumpProvider.class,
                            VmInfo.class,
                            IProgressListener.class,
                            IProgressListener.OperationCanceledException.class,
                            Severity.class,
                            Messages.class,
            };
    ...
    /**
     * Ask the user for a suitable Java home for an IBM VM to help attach to the target VM.
     * @param oldHome
     * @return
     */
