...
public class OQLContentAssistantProcessor implements IContentAssistProcessor {
	...
	/**
	 * Asks the extractor to get the context, then gets the list of suggestions through
	 * the provider and build the ICompletionProposal array to be returned.
	 * @param arg0 is the text viewer we are working on
	 * @param arg1 is the current position.
	 */
	...
	/**
	 * Builds the results in the form of ICompletionProposal[] from the list of Strings.
	 * @param suggestions the list of suggestions
	 * @param arg1 is the position where to place the suggestion
	 * @param replaceLength is the substring to be substituted length
	 * @return
	 */
	private ICompletionProposal[] buildResult(List<ContentAssistElement> suggestions, int currentCursor, int replaceLength) {
		...
		while (it.hasNext()){
			...
			ICompletionProposal completion = new CompletionProposal(
					classname, 
					currentCursor - replaceLength, 
					replaceLength, 
					currentCursor - replaceLength + classname.length());
