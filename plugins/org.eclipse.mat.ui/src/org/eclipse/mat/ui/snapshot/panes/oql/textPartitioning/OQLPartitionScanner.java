 ...
 * SELECT, FROM, WHERE, UNION clauses.
 * 
...
public class OQLPartitionScanner extends RuleBasedPartitionScanner {
	...
	public OQLPartitionScanner() {
		...
		rules[0] = new MultilineNonConsumingRule("SELECT", new String[]{"FROM","/*","//"}, selectT);
		
		rules[1] = new MultilineNonConsumingRule("FROM", new String[]{"UNION","WHERE","SELECT","/*","//"}, fromT);
		
		rules[2] = new MultilineNonConsumingRule("WHERE", new String[]{"UNION","SELECT","/*","//"}, whereT);
		
		rules[3] = new MultilineNonConsumingRule("UNION", new String[]{"SELECT","/*","//"}, unionT);
		
		rules[4] = new MultiLineRule("/*", "*/", commentT);
