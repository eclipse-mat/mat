...
public class TextOutputter extends OutputterBase implements IOutputter
{
    ...
    private static class RefinedTreeTextEmitter extends StructuredResultTextEmitter
    {
        ...
        protected Object[] getItems()
        {
            ...
            List<?> elements2 = tree.getElements();
            if (!elements.equals(elements2))
                System.out.println("Bad elements "+tree); //$NON-NLS-1$
            for (Object o : elements)
            {
                if (tree.hasChildren(o))
                {
                    List<?>children1 = tree.getChildren(o);
                    List<?>children2 = tree.getChildren(o);
                    if (!children1.equals(children2)) {
                        System.out.println("Bad children "+tree); //$NON-NLS-1$
                        break;
                    }
                }
            }
