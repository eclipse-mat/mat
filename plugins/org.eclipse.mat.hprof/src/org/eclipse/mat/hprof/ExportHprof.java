...
public class ExportHprof implements IQuery
{
    ...
    @Argument(isMandatory = false, advice = Advice.SAVE, flag = "remap")
    public File remapFile;
    ...
    public IResult execute(IProgressListener listener) throws Exception
    {
        ...
        remap.saveRemap(remapFile, undo, comments);
                    ...
                    throws IOException
    {
        ...
        long mark = os2.size();
        writeID(os2, id);
        byte utf[] = ss.getBytes(Charset.forName("UTF-8")); //$NON-NLS-1$
        os2.write(utf);
    ...
    private void writeField(DataOutput os, Field fld, boolean addType) throws IOException
    {
        ...
        switch (ty)
        {
            ...
            case IObject.Type.BYTE:
                if (addType)
                    os.writeByte(IObject.Type.BYTE);
                byte byteValue = redact != RedactType.NONE ? 0 : ((Byte) fld.getValue()).byteValue();
                os.writeByte(byteValue);
                break;
            case IObject.Type.CHAR:
                if (addType)
                    os.writeByte(IObject.Type.CHAR);
                char charValue = redact != RedactType.NONE ? 0 : ((Character) fld.getValue()).charValue();
    ...
    private boolean dumpPrimitiveArray(DataOutputStream3 os, IObject io, boolean check) throws IOException
    {
        ...
        else if (ii.getType() == IObject.Type.CHAR)
        {
            for (int i = 0; i < ii.getLength(); ++i)
            {
                char shortValue = redact != RedactType.NONE ? 0 : ((char[]) a)[i];
        ...
        else if (ii.getType() == IObject.Type.INT)
        {
            for (int i = 0; i < ii.getLength(); ++i)
            {
                int intValue = redact != RedactType.NONE ? 0 : ((int[]) a)[i];
                os.writeInt(intValue);
            }
        }
    ...
    public static class Remap {
         ...
         */
        public Remap(Pattern skipPattern, Pattern avoidPattern, boolean undo)
        {
         ...
         * @throws IOException
         */
        public void loadRemap(File remapFile, boolean undo) throws IOException
        {
            if (remapFile != null && remapFile.canRead())
            {
                Properties p = new Properties();
                FileReader rdr = new FileReader(remapFile);
                try {
                    p.load(rdr);
                }
                finally
                ...
                for (Map.Entry<Object,Object> e : p.entrySet())
                {
                    ...
                    {
                        // Reverse the mapping
                        obfuscated.put(e.getValue().toString(), e.getKey().toString());
                    }
                    else
                    {
                        // Mapping file shows old to new
                        obfuscated.put(e.getKey().toString(), e.getValue().toString());
                    }
                }
            }
        }

        public void saveRemap(File remapFile, boolean undo, String comments) throws IOException
        {
            // Only save if we are not doing a reverse mapping, and it is okay to write to the file
            if (!undo && remapFile != null && (remapFile.canWrite() || !remapFile.exists()))
            {
                ...
                FileWriter wrt = new FileWriter(remapFile);
                try {
        ...
        public String renameMethodname(String className, String method, boolean upper)
        {
            ...
            final String methodSep = "@"; //$NON-NLS-1$
        ...
        public String renameClassname(String classname)
        {
            ...
            // If only com.sun. is renamed, don't rename com.
            if (!isRemapped(classname))
                return classname;
            String pack;
            String newpack;
        ...
        private String remap(String classname, String newpack, String cn, String last, boolean field, boolean upper)
        {
            for (int k = 0; k < 150; ++k)
            {
                ...
                if (!used.contains(np) && isRemapped(np) && !isAvoid(np))
                {
                    // found new mapping
        ...
        private String randomWordLen(int length)
        {
            ...
            do {
                ...
                while (ret.length() != (vowel ? length : length - 1));
        ...
        /**
         * length varies from 2*parts+1 to 4*part+2s
         * 
         * @param parts
         * @return random word
         */
