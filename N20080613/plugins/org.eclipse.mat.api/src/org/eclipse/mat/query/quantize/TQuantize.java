...
import org.eclipse.mat.query.quantize.Quantize.Bucket;
...
public final class TQuantize
{
                    ...
                    throws SnapshotException
    {
        ...
        if (columns.length > 1)
        {
            Builder builder = new Builder(snapshot, base, new ByMultipleColumns(columns));
        ...
        {
            Builder builder = new Builder(snapshot, base, new BySingleColumn(columns[0]));
    ...
    interface GroupedRow extends Bucket, Comparable<GroupedRow>
    {
        List<?> getSubjects();
            ...
            if (element instanceof GroupedRowImpl)
            {
                ...
                switch (columnIndex)
                {
                    ...
                    default:
                        ...
                        return function != null ? function.getValue(row) : null;
                }
            }
            else
            {
                ...
                while (p != null)
                {
                    ...
                    for (int ii = 0; ii < p.functions.length; ii++)
                    {
                        if (p.functions[ii] != null)
                            p.functions[ii].add(row.functions[ii].getValue(row));
                    }
                    p = p.parent;
                }
        ...
        public Object getColumnValue(Object element, int columnIndex)
        {
            if (element instanceof Package)
            {
                ...
                else
                {
                    ...
                    return function != null ? function.getValue(p) : null;
                }
            }
            else
            {
    ...
    static class BySingleColumn extends Calculator
    {
        ...
        public BySingleColumn(int columnIndex)
        {
    ...
    static class ByMultipleColumns extends Calculator
    {
        ...
        public ByMultipleColumns(int... columnIndex)
        {
        ...
        {
            // TODO use formatter
            return String.valueOf(((CompositeKey) key).keys[0]);
        }
    ...
    private static ResultMetaData wrap(IResultTable table)
    {
        ...
        if (data.isShowPreciseRetainedSize())
            answer.setShowPreciseRetainedSizes();
        if (data.isShowApproximateRetainedSize())
            answer.setShowApproximateRetainedSizes();
